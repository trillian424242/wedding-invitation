export const WEDDING_INVITATION_URL = 'https://wedding-invitation-woad-tau.vercel.app';
export const KAKAOTALK_API_TOKEN = '8a5469499cef54cf2855eb3a3e7f6711';
export const KAKAOTALK_SHARE_IMAGE = 'https://wedding-invitation-woad-tau.vercel.app/assets/sea_black_and_white_2.jpg';
export const WEDDING_DATE = '2023년 9월 23일, 토요일 오전 11시';
export const WEDDING_LOCATION = '더 컨벤션 잠실점 비스타홀';

export const GROOM_NAME = '조원일';
export const GROOM_ACCOUNT_NUMBER = '신한 ***-***-******';
export const GROOM_FATHER_NAME = '조유철';
export const GROOM_FATHER_ACCOUNT_NUMBER = '우리 ****-***-******';
export const GROOM_MOTHER_NAME = '박순석';
export const GROOM_MOTHER_ACCOUNT_NUMBER = 'NH농협 ***-**-******';

export const BRIDE_NAME = '조윤진';
export const BRIDE_ACCOUNT_NUMBER = '신한 ***-***-******';
export const BRIDE_FATHER_NAME = '조호진';
export const BRIDE_FATHER_ACCOUNT_NUMBER = '신한 ***-**-******';
export const BRIDE_MOTHER_NAME = '김정화';
export const BRIDE_MOTHER_ACCOUNT_NUMBER = '○○은행 ***-***-******';
